...
export class AuthService {
    ...
    login(loginName: string, password: string) {
        ...
        return this.loginUser(body.toString());

        // this.loginUser(body.toString()).subscribe(data => {
    ...
    loginUser(loginPayload) {
        const headers = {
          'Authorization': 'Basic ' + btoa('citi-client:citi-secret'),
    ...
    getAllUsers(): Observable<User[]> {
       ...
       // return this.http.get<User[]>(this.baseUrl + 'user/auth/users?access_token=' + JSON.parse(window.sessionStorage.getItem('token')).access_token);
    }
